...
1. Open the [SonarCloud project](https://sonarcloud.io/project/issues?id=joe-tingsanchali-sonarsource_sn-tf-vm-pub).
2. Select the **`feature/backdating-demo`** branch from the branch dropdown.
3. Navigate to **Issues** and filter by file: `demo_backdating.tf`.
4. Click on any issue — check the **"Introduced"** date.
5. **Expected result**: The "Introduced" date shows **January 15, 2023**, not today's date.

This confirms that SonarQube used `git blame` to backdate the issue to the commit date of the line, rather than stamping it with the analysis date.
